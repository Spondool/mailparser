...
class MailParser extends Transform {
    ...
    createNode(node) {
        ...
        if (!/^multipart\//i.test(contentType)) {
            ...
            if (node.filename || !disposition && !['text/plain', 'text/html'].includes(contentType)) {
                newNode.disposition = 'attachment';
            } else {
                newNode.disposition = disposition || 'inline';
            }

            newNode.isAttachment = node.filename || !['text/plain', 'text/html'].includes(contentType) || newNode.disposition !== 'inline';
